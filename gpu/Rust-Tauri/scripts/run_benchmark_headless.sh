...
echo "=== RTSP 30-Stream GPU Zero-Copy Benchmark (Rust Tauri) ==="
...
# Build frontend and binary if release binary does not exist
BINARY_PATH="$DIR/src-tauri/target/release/rtsp-stress-test-tauri-gpu"
if [ ! -f "$BINARY_PATH" ]; then
  echo "[*] Building Rust Tauri GPU benchmark application (Release)..."
  npm run build
fi
...
echo "[*] Launching Tauri GPU application..."

# On Linux, run inside Xvfb with GPU hardware acceleration flags
if [[ "$OSTYPE" == "linux"* ]]; then
  export WEBKIT_FORCE_COMPOSITING_MODE=1
  export GST_VAAPI_ALL_DRIVERS=1
  export WEBKIT_DISABLE_DMABUF_RENDERER=0

  if command -v nvidia-smi >/dev/null 2>&1 || [ -f "/usr/lib/x86_64-linux-gnu/dri/nvidia_drv_video.so" ]; then
    export LIBVA_DRIVER_NAME="${LIBVA_DRIVER_NAME:-nvidia}"
    export __GLX_VENDOR_LIBRARY_NAME="${__GLX_VENDOR_LIBRARY_NAME:-nvidia}"
    echo "[*] NVIDIA GPU detected. Hardware driver flags configured."
  fi

  echo "[*] Launching under Xvfb (2560x1440x24) with hardware acceleration flags..."
