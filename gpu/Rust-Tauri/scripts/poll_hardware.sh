...
if [ -z "$TARGET_PID" ]; then
  # Auto-detect Rust Tauri GPU benchmark process if PID not supplied
  TARGET_PID=$(pgrep -f "rtsp-stress-test-tauri-gpu" | head -n 1)
fi

if [ -z "$TARGET_PID" ]; then
  echo "Usage: $0 <PID>"
  echo "Or start the Tauri app first to allow auto-detection."
  exit 1
fi
...
while kill -0 "$TARGET_PID" 2>/dev/null; do
  ...
  # In GPU benchmark, query nvidia-smi for VRAM allocation and NVDEC decoder utilization
  GPU_VRAM_MB="0"
