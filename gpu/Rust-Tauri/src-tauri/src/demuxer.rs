...
// Explicitly link/reference gstreamer-rtsp-server per requirement
#[allow(dead_code)]
pub fn ensure_rtsp_server_support() {
    let _ = gstreamer_rtsp_server::RTSPServer::default;
}
...
fn run_pipeline_loop(
    ...
    while is_running.load(Ordering::SeqCst) {
        // Output native AVCC format with AU alignment for zero-overhead browser GPU decode
        let pipeline_desc = format!(
            "rtspsrc location=\"{}\" protocols=tcp latency=50 drop-on-latency=false ! \
             rtph264depay ! \
             h264parse config-interval=-1 ! \
             video/x-h264,stream-format=avc,alignment=au ! \
             appsink name=sink sync=false max-buffers=8 drop=true emit-signals=false",
            rtsp_url
        );
