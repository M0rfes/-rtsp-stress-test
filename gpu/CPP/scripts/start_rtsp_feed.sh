...
if [ -n "$INPUT_VIDEO" ] && [ -f "$INPUT_VIDEO" ]; then
  ...
  ffmpeg -re -stream_loop -1 -i "$INPUT_VIDEO" \
    -c:v libx264 -profile:v high -level:v 5.1 -preset ultrafast -tune zerolatency \
  ...
  # Generate 1440p 25fps test pattern with repeatable in-band SPS/PPS headers and Level 5.1
  ffmpeg -re -f lavfi -i "testsrc2=size=2560x1440:rate=25" \
    -c:v libx264 -profile:v high -level:v 5.1 -preset ultrafast -tune zerolatency -threads 4 \
