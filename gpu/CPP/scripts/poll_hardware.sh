...
if [ -z "$TARGET_PID" ]; then
  # Auto-detect C++ Qt6 GPU benchmark process if PID not supplied
  TARGET_PID=$(pgrep -f "rtsp-stress-test-cpp-gpu" | head -n 1)
fi

if [ -z "$TARGET_PID" ]; then
  echo "Usage: $0 <PID>"
  echo "Or start the C++ Qt6 GPU app first to allow auto-detection."
  exit 1
fi
...
while kill -0 "$TARGET_PID" 2>/dev/null; do
  ...
  # Query NVIDIA GPU metrics if nvidia-smi is available
