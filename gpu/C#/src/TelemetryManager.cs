...
public sealed class TelemetryPayload
{
    ...
    public string HardwareMode { get; set; } = "gpu";
    ...
    public void Tick(IReadOnlyList<StreamWorker> workers)
    {
        lock (_lock)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                live++;
                var scored = paintedCur > 0 ? paintedCur : decodedCur;
                var prevScored = paintedCur > 0 ? _prevPaintedFrames[i] : _prevDecodedFrames[i];
                var deltaP = (paintedCur >= _prevPaintedFrames[i]) ? (uint)(paintedCur - _prevPaintedFrames[i]) : (uint)paintedCur;
                var deltaD = (decodedCur >= _prevDecodedFrames[i]) ? (uint)(decodedCur - _prevDecodedFrames[i]) : (uint)decodedCur;
                var deltaScored = (scored >= prevScored) ? (uint)(scored - prevScored) : (uint)scored;
                ...
                worker.CurrentFps = deltaScored;
                totalFps += deltaScored;

                _paintedBuckets.AddSample(deltaScored);
