#!/usr/bin/env bash
# poll_hardware.sh - External OS Hardware Polling for C# Avalonia GPU Benchmark
...
if [ -z "$TARGET_PID" ]; then
  TARGET_PID=$(pgrep -f "rtsp-stress-test-csharp-gpu" | head -n 1)
fi

if [ -z "$TARGET_PID" ]; then
  echo "Usage: $0 <PID>"
  echo "Or start the C# Avalonia GPU app first to allow auto-detection."
  exit 1
fi
