#!/usr/bin/env bash
# setup_autostart.sh - Configures systemd autostart for RTSP GPU Benchmark on Ubuntu EC2
set -e

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CURRENT_USER="$(whoami)"
SERVICE_SRC="$DIR/scripts/rtsp-benchmark-gpu.service"
SERVICE_DEST="/etc/systemd/system/rtsp-benchmark-gpu.service"

echo "=== Configuring Autostart Service for RTSP GPU Zero-Copy Benchmark ==="
...
sed -e "s|User=ubuntu|User=$CURRENT_USER|g" \
    ...
    -e "s|/opt/rtsp-stress-test/gpu/Electron|$DIR|g" \
...
sudo systemctl enable rtsp-benchmark-gpu.service
...
echo ""
echo "=== Setup Complete! ==="
echo "The GPU benchmark is now running and will automatically start on every box reboot."
echo "View live service logs:   journalctl -u rtsp-benchmark-gpu -f"
...
echo "View GPU engine load:     nvidia-smi dmon"
