...
if [ -z "$TARGET_PID" ]; then
  # Auto-detect Electron GPU process if PID not supplied
  TARGET_PID=$(pgrep -f "rtsp-stress-test-electron-gpu" | head -n 1)
  if [ -z "$TARGET_PID" ]; then
    TARGET_PID=$(pgrep -f "electron" | head -n 1)
  fi
fi

if [ -z "$TARGET_PID" ]; then
  echo "Usage: $0 <PID>"
  echo "Or start the Electron app first to allow auto-detection."
  exit 1
fi

echo "Starting hardware polling (GPU mode) for PID: $TARGET_PID"
...
while kill -0 "$TARGET_PID" 2>/dev/null; do
  ...
  # In GPU benchmark, query nvidia-smi for VRAM allocation and NVDEC utilization
  GPU_VRAM_MB="0"
