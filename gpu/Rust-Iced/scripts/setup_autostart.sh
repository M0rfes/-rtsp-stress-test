#!/usr/bin/env bash
# setup_autostart.sh - Configures systemd autostart for RTSP Rust Iced GPU Benchmark on Ubuntu EC2
set -e

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CURRENT_USER="$(whoami)"
SERVICE_SRC="$DIR/scripts/rtsp-benchmark-iced-gpu.service"
SERVICE_DEST="/etc/systemd/system/rtsp-benchmark-iced-gpu.service"

echo "=== Configuring Autostart Service for RTSP Rust Iced GPU Benchmark ==="
...
sed -e "s|User=ubuntu|User=$CURRENT_USER|g" \
    ...
    -e "s|/opt/rtsp-stress-test/gpu/Rust-Iced|$DIR|g" \
...
sudo systemctl enable rtsp-benchmark-iced-gpu.service

echo "[*] Starting service now..."
sudo systemctl restart rtsp-benchmark-iced-gpu.service
...
echo "View live service logs:   journalctl -u rtsp-benchmark-iced-gpu -f"
