...
if [ -z "$TARGET_PID" ]; then
  # Auto-detect Rust Iced GPU benchmark process if PID not supplied
  TARGET_PID=$(pgrep -f "rtsp-stress-test-iced-gpu" | head -n 1)
fi
...
while kill -0 "$TARGET_PID" 2>/dev/null; do
  ...
  # Poll GPU metrics via nvidia-smi if available
  GPU_VRAM_MB="0"
