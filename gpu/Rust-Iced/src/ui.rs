...
pub fn view_hud<'a>(hud: &'a HudState) -> Element<'a, Message> {
    ...
    let badge_framework = make_badge("Rust Iced GPU (Zero-Copy)", Color::from_rgb8(45, 110, 240));
    let badge_backend = make_badge("iced_wgpu", Color::from_rgb8(34, 160, 90));
    let badge_decoder = make_badge(
        if hud.decoder_plugin == "nvdec" { "NVIDIA NVDEC" } else if hud.decoder_plugin == "vtdec" { "Apple VideoToolbox" } else { "GStreamer H.264" },
        Color::from_rgb8(140, 70, 210)
    );
    let badge_vram = make_badge("VRAM Shader Blit", Color::from_rgb8(210, 130, 20));
...
pub fn view_camera_tile<'a>(slot: &'a std::sync::Arc<StreamSlot>, stream_fps: u32) -> Element<'a, Message> {
    ...
    let has_frame = slot.frame.load().is_some();

    let shader_element: Element<'a, Message> = video_tile(slot.clone())
        .width(Length::Fill)
        .height(Length::Fill)
        .into();

    let content: Element<'a, Message> = if has_frame {
        shader_element
    } else {
        let status_str = if is_connected { "Decoding (VRAM)..." } else { "Connecting..." };
