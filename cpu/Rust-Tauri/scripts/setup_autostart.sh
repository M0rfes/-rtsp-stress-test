#!/usr/bin/env bash
# setup_autostart.sh - Configures systemd autostart for RTSP Rust Tauri CPU Benchmark on Ubuntu EC2
set -e

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CURRENT_USER="$(whoami)"
SERVICE_SRC="$DIR/scripts/rtsp-benchmark-tauri-cpu.service"
SERVICE_DEST="/etc/systemd/system/rtsp-benchmark-tauri-cpu.service"

echo "=== Configuring Autostart Service for RTSP Rust Tauri CPU Benchmark ==="
...
sed -e "s|User=ubuntu|User=$CURRENT_USER|g" \
    ...
    -e "s|/opt/rtsp-stress-test/cpu/Rust-Tauri|$DIR|g" \
...
sudo systemctl enable rtsp-benchmark-tauri-cpu.service

echo "[*] Starting service now..."
sudo systemctl restart rtsp-benchmark-tauri-cpu.service

echo "[*] Service status:"
sudo systemctl status rtsp-benchmark-tauri-cpu.service --no-pager

echo ""
echo "=== Setup Complete! ==="
echo "The benchmark is now running and will automatically start on every box reboot."
echo "View live service logs:   journalctl -u rtsp-benchmark-tauri-cpu -f"
