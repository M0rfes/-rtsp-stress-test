#!/usr/bin/env bash
# poll_hardware.sh - External OS Hardware Polling for 6-Hour RTSP Video Grid Benchmark
...
if [ -z "$TARGET_PID" ]; then
  # Auto-detect C++ Qt6 benchmark process if PID not supplied
  TARGET_PID=$(pgrep -f "rtsp-stress-test-cpp-cpu" | head -n 1)
fi

if [ -z "$TARGET_PID" ]; then
  echo "Usage: $0 <PID>"
  echo "Or start the C++ Qt6 app first to allow auto-detection."
  exit 1
fi
...
while kill -0 "$TARGET_PID" 2>/dev/null; do
  ...
  # In CPU-only benchmark, GPU metrics remain 0 per specification
  GPU_VRAM_MB="0"
  GPU_DECODER_PERCENT="0"

  # Optional check if nvidia-smi exists
