#!/usr/bin/env bash
# setup_autostart.sh - Configures systemd autostart for RTSP C++ Qt6 CPU Benchmark on Ubuntu EC2
set -e

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CURRENT_USER="$(whoami)"
SERVICE_SRC="$DIR/scripts/rtsp-benchmark-cpp-cpu.service"
SERVICE_DEST="/etc/systemd/system/rtsp-benchmark-cpp-cpu.service"

echo "=== Configuring Autostart Service for RTSP C++ Qt6 CPU Benchmark ==="
...
sed -e "s|User=ubuntu|User=$CURRENT_USER|g" \
    ...
    -e "s|/opt/rtsp-stress-test/cpu/CPP|$DIR|g" \
...
sudo systemctl enable rtsp-benchmark-cpp-cpu.service

echo "[*] Starting service now..."
sudo systemctl restart rtsp-benchmark-cpp-cpu.service
...
echo "View live service logs:   journalctl -u rtsp-benchmark-cpp-cpu -f"
