...
if [ -z "$TARGET_PID" ]; then
  # Auto-detect C# benchmark process if PID not supplied
  TARGET_PID=$(pgrep -f "rtsp-stress-test-csharp-cpu" | head -n 1)
fi

if [ -z "$TARGET_PID" ]; then
  echo "Usage: $0 <PID>"
  echo "Or start the C# Avalonia app first to allow auto-detection."
  exit 1
fi
