#!/usr/bin/env bash
# run_benchmark_headless.sh - 6-Hour Benchmark Runner for Headless Linux (AWS Ubuntu) & Local Testing
set -e
...
echo "=== RTSP 30-Stream CPU Benchmark (C# Avalonia - FFmpeg.AutoGen) ==="
...
# Publish / build optimized release binary if not present
PUBLISH_DIR="$DIR/bin/publish"
APP_DLL="$PUBLISH_DIR/rtsp-stress-test-csharp-cpu.dll"
APP_BIN="$PUBLISH_DIR/rtsp-stress-test-csharp-cpu"

if [ ! -f "$APP_DLL" ] && [ ! -f "$APP_BIN" ]; then
  echo "[*] Publishing C# Avalonia benchmark application (Release)..."
...
echo "[*] Launching C# Avalonia CPU application (software decoding backend)..."

CMD=()
if [ -f "$APP_BIN" ] && [ -x "$APP_BIN" ]; then
  CMD=("$APP_BIN" --url "$RTSP_URL" --streams "$STREAM_COUNT" --log-dir "$LOG_DIR")
else
  CMD=(dotnet "$APP_DLL" --url "$RTSP_URL" --streams "$STREAM_COUNT" --log-dir "$LOG_DIR")
fi

# On Linux, run inside Xvfb with software rendering flags
